...
## 2026-08-27 — `/map`, `/zipterior` 푸터 제거 후에도 남아있던 1px 스크롤 수정

### 시작 전
- 사용자가 이전 턴의 푸터 제거는 잘 됐다고 확인하면서, 스크린샷 첨부:
  화면 우측 끝에 아주 얇은 스크롤바가 미세하게 보임 — "오른쪽에 보면
  미세하게 사이즈 때문에 스크롤이 생긴거 같은데 이부분 조정하자."

### 진행 중
- **원인**: `Header.tsx`의 `<header>`가 `border-b`(1px)를 갖고 있는데,
  실제 높이(`h-16`)는 그 안의 `<div>`에만 적용돼 있어서 헤더 전체
  실측 높이가 `4rem + 1px`. `/map`/`/zipterior`는
  `h-[calc(100vh-4rem)]`로 헤더 높이를 정확히 4rem으로 가정하고
  있어서, 실제로는 문서 전체 높이가 뷰포트보다 딱 1px 넘쳐 브라우저가
  아주 얇은 스크롤바를 계속 그리고 있었음(어제 만든 "이제 5rem 언저리
  쓰지 말고 4rem 그대로" 로직과는 무관, 순수 보더 폭 미계산).
- **[완료]** `calc()` 픽셀을 억지로 맞추는 대신(보더 폭이 나중에 또
  바뀔 수 있어 취약함), `Footer.tsx`에 `useEffect`를 추가해 이 두
  라우트에 있는 동안은 `document.body.style.overflow = "hidden"`으로
  body 자체 스크롤을 원천 차단 — 어떤 서브픽셀 오차가 있어도 스크롤바
  자체가 안 뜨게 함. 라우트를 벗어나면 원래 overflow 값으로 복원.
  기존 "이 두 라우트에서는 Footer를 렌더링 안 함" 로직과 같은 pathname
  판정(`FULLSCREEN_ROUTE_PREFIXES`)을 그대로 재사용.
- **[완료] 검증**: `next build` 클린, `npx eslint` 새 오류 없음.
  Playwright로 `/map`에서 `getComputedStyle(document.body).overflow`
  가 `"hidden"`, 홈(`/`)에서는 `"visible"`로 정상 복원되는 것 확인.

### 후속: 인테리어 위젯을 "레이어 패널"과 동일한 버튼→드롭다운 방식으로, 목록 2열 배치
- 사용자가 지도 레이어 패널 스크린샷을 보여주며 2건 요청:
  1. "인테리어" 위젯도 저 레이어 버튼처럼, 버튼을 누르면 그 바로
     아래로 드롭다운이 나오게 하고, 눌렀을 때 버튼이 빨간색으로
     바뀌는 것도 적용해달라(지금은 항상 떠 있다가 닫으면 상단에
     아이콘만 남는 구조였음 — 완전히 다른 상호작용 패턴).
  2. 위젯 안 목록이 세로로 나열되어 스크롤이 생기니 2열 배치로
     바꿔달라.
- **[완료] 수정 1**: `map/page.tsx`의 상태를 `portfolioWidgetClosed`
  (기본 열림, 닫으면 아이콘만 남음)에서 `portfolioPanelOpen`(기본
  닫힘)으로 바꾸고, 레이어 버튼과 완전히 같은 구조로 재작성 —
  `<div className="relative">` 안에 토글 버튼(`controlButtonClass
  (portfolioPanelOpen)` 그대로 재사용 → 열리면 다른 켜진 컨트롤과
  동일하게 빨간 테두리/글자로 강조)과, 열렸을 때만 렌더링되는
  `<NearbyPortfolioWidget>`을 넣음. 위젯 하단에 별도로 떠 있던
  기존 렌더링(및 그 안내 주석)은 제거 — 이제 위젯은 항상 이 버튼의
  드롭다운으로만 존재한다.
- **[완료] 수정 2**: `NearbyPortfolioWidget`의 루트 위치를
  `absolute bottom-4 right-4`(독립 플로팅)에서 `absolute right-0
  top-12`(레이어 패널과 동일 — 버튼 바로 아래 붙음)로 변경, 폭도
  `w-72`→`w-80`.
- **[완료] 수정 3(2열 배치)**: 처음엔 정사각 썸네일 카드로 2열
  그리드를 만들었다가, 계산해보니 카드 한 줄이 오히려 기존 세로
  목록 한 줄보다 커져서(정사각 썸네일 높이 때문) 총 높이가 더
  늘어나는 역효과라는 걸 확인 — **폐기**. 대신 기존 "작은 정사각
  썸네일(h-10 w-10) + 텍스트" 가로 카드를 그대로 두고 `grid
  grid-cols-2`로 2개씩만 나란히 배치 — 한 줄 높이는 그대로 유지한
  채 줄 수만 절반으로 줄여 5개 항목이 `max-h-80` 안에 스크롤 없이
  들어가게 함.
- **[완료] 검증**: `next build` 클린. `npx eslint
  NearbyPortfolioWidget.tsx src/app/map/page.tsx` — 오류 6건, 전부
  기존 베이스라인과 동일(신규 없음).
- **미검증**: 브라우저에서 (1) 인테리어 버튼 클릭 시 레이어 버튼과
  같은 위치/느낌으로 드롭다운이 열리고 버튼이 빨간색으로
  바뀌는지, (2) 목록이 2열로 스크롤 없이 표시되는지, (3) 좁은
  텍스트 영역(카드 폭이 절반으로 줄어듦)에서 단지명/평형 표시가
  과도하게 잘리지 않는지 실사용 확인 필요 — 잘림이 심하면 폭을
  `w-80`에서 더 넓히거나 텍스트를 1줄로 줄이는 추가 조정 검토.

### 후속: 레이어/인테리어 패널 동시열림 방지, 18개 3페이지×6개 가로 스와이프, 버튼 텍스트 강조
- 사용자가 스크린샷(레이어 패널 + 인테리어 드롭다운이 동시에 겹쳐
  보이는 상태)을 보여주며 3건 요청:
  1. 레이어 패널과 인테리어 패널이 동시에 열리지 않게 — 새 위젯
     버튼을 누르면 기존에 열려 있던 다른 위젯은 자동으로 닫히도록.
  2. 인테리어 위젯 포트폴리오를 6개씩 노출, 총 3페이지(18개)를
     하단 점 3개로 표시하며 스크롤로 다음 페이지를 넘겨보는 구조로.
     점은 밝은 회색 바탕에 현재 페이지만 짙은 회색(크기도 살짝
     크게).
  3. 버튼의 "인테리어" 글씨 크기를 살짝 키우고 볼드 처리.
- **[완료] 수정 1(상호 배타)**: `map/page.tsx`의 레이어 버튼
  onClick에 `setPortfolioPanelOpen(false)`를, 인테리어 버튼
  onClick에 `setLayerPanelOpen(false)`를 추가 — 각자 자기 상태는
  토글하면서 상대방은 항상 닫아, 두 패널이 동시에 열려 있는 상태
  자체가 나오지 않게 함(레이어 패널 안의 자체 닫기(×) 버튼은
  그대로 두어 인테리어 상태는 안 건드림).
- **[완료] 수정 2(6개×3페이지 가로 스와이프)**: 기존 세로 목록/2열
  그리드(최대 5~6개)를 걷어내고, `NearbyPortfolioWidget`에
  `PAGE_SIZE=6`, `PAGE_COUNT=3`(→`FEED_LIMIT=18`)로 확장 —
  백엔드 `/portfolios/feed`의 `limit` 상한이 이미 20이라 API 변경
  없이 한 번의 요청으로 18개를 받아옴. 받은 배열을 `chunk()`로
  6개씩 3페이지로 나누고, `overflow-x-auto snap-x snap-mandatory`
  가로 스크롤 컨테이너에 페이지마다 `w-full shrink-0 snap-center`
  div(2열×3행 그리드)를 나란히 배치 — 스크롤하면 페이지 단위로
  스냅되어 넘어간다. `onScroll`에서 `scrollLeft/clientWidth`로
  현재 페이지 인덱스를 계산해 하단 점에 반영. 탭 전환 시
  `switchTab()`에서 페이지를 0으로 리셋하고 스크롤 위치도
  `scrollTo({left:0})`로 되돌림(안 하면 "최근 등록"에서 2페이지를
  보다가 "우리집과 가까운"으로 넘어가도 스크롤 위치가 그대로 남는
  문제). 점 색상은 `bg-line`(밝은 회색, 비활성) / `bg-ink/70`(짙은
  회색, 활성)이고 활성 점만 `h-2.5 w-2.5`로 비활성(`h-2 w-2`)보다
  살짝 크게.
- **[완료] 수정 3**: 인테리어 버튼 텍스트 클래스를 `text-[10px]
  font-bold` → `text-[11px] font-extrabold`로.
- **[완료] 검증**: `next build` 클린. `npx eslint
  NearbyPortfolioWidget.tsx src/app/map/page.tsx` — 오류 6건, 전부
  기존 베이스라인과 동일(신규 없음). 백엔드는 변경 없음(스키마 API
  `limit` 파라미터가 이미 20까지 허용 — `ge=1, le=20` 확인).
- **미검증**: 브라우저에서 (1) 레이어/인테리어 버튼이 서로 배타적으로
  열리는지, (2) 가로 스와이프로 3페이지가 정확히 스냅되며 넘어가고
  점 표시가 맞는지, (3) 총 18개가 안정적으로 채워지는지(실제 등록된
  포트폴리오가 18개 미만인 지역은 페이지 수가 자동으로 줄어드는 게
  정상 동작).

### 후속: 드롭다운 위치를 버튼 아래→왼쪽으로, 페이지 점 크기 축소
- 사용자 요청 2건: (1) 인테리어 위젯 하단 페이지 점 3개가 너무 커서
  살짝 작게, (2) 레이어/인테리어 드롭다운이 지금 버튼 "아래"로
  펼쳐지는데, 버튼 "왼쪽"으로(간격 살짝 두고) 펼쳐지도록.
- **[완료] 수정 1**: 레이어 패널(`map/page.tsx`)과
  `NearbyPortfolioWidget` 루트의 위치 클래스를 `absolute right-0
  top-12`(버튼 아래, 오른쪽 정렬)에서 `absolute right-full top-0
  mr-2`로 변경 — `right-full`(`right: 100%`)이 드롭다운의 오른쪽
  끝을 버튼을 감싼 `relative` 컨테이너의 왼쪽 끝에 맞춰 버튼 왼쪽
  전체로 밀어내고, `top-0`으로 버튼과 위쪽을 맞추고, `mr-2`로
  버튼과의 간격을 살짝 둠. 두 위젯 모두 우측 상단 컨트롤 스택 안의
  버튼을 감싼 `relative` div 폭이 버튼 폭(44px)과 같아서
  `right-full` 기준이 정확히 그 버튼 기준으로 잡힌다.
- **[완료] 수정 2**: 페이지 점 크기를 활성 `h-2.5 w-2.5`→`h-2 w-2`,
  비활성 `h-2 w-2`→`h-1.5 w-1.5`로 한 단계씩 축소(상대적 크기 차이는
  유지).
- **[완료] 검증**: `next build` 클린, eslint 신규 오류 없음(기존
  베이스라인 6건과 동일).
- **미검증**: 브라우저에서 드롭다운이 화면 밖으로 잘리지 않고 버튼
  왼쪽에 잘 붙어 보이는지 확인 필요(지도가 넓어 왼쪽 공간은
  충분할 것으로 예상하지만 실측 필요).
...
## 2026-08-28 — `/map` 왼쪽 홈 사이드바 ("부동산 사이클" 컨셉) 신규 추가

### 배경
Claude Design 캔버스 목업으로 여러 라운드 반복(집품/KB부동산 레퍼런스
참고 → "매도→매수→인테리어→이사·정착" 부동산 사이클을 시각화하는 방향으로
사용자와 합의)한 뒤, "일단 목업 반영시키자, 수정은 나중에"라는 사용자
지시로 실제 코드에 반영. 지금까지는 `/map`이 지도 하나로 꽉 찬 화면이라
집팔고360이 매물 지도 서비스로만 보였는데, 이번에 왼쪽에 상시 노출되는
홈 패널을 추가해서 집팔고/집사고/집테리어/집서비스를 한 화면에서 순환
흐름으로 보여주도록 바꿈.

### 구현
- 신규 컴포넌트 `apps/web/src/components/map/HomeSidebar.tsx`: 왼쪽
  세로 아이콘 레일(홈/매물/구매의뢰/집서비스/지도/메뉴) + 탭별 패널.
  "홈" 탭엔 부동산 사이클 다이어그램(4단계 클릭 선택) + 선택한 단계별
  강조 CTA 카드 + 안전정보/시공사례 미리보기/최근 조회 랭킹/시황 한 줄/
  후기. CTA 클릭 시 뜨는 입력폼→완료 모달(FlowModal)까지 전부 이
  파일 안에 있음. **매물/구매의뢰/집서비스 탭 콘텐츠, 최근 조회 랭킹,
  시황 수치는 전부 정적 자리표시 데이터** — 아직 실제 API에 연결 안 함
  (다음 손볼 항목).
- 신규 컴포넌트 `apps/web/src/components/map/PromoOverlays.tsx`: 지도
  우하단 배너 + 가운데 모달. 마케팅 배너 실제 이미지 파일(사용자가 압축
  파일로 업로드한 것)을 `apps/web/public/images/promo/`에 저장해서 그대로
  사용 — `zippalgo-ai-banner.webp`, `zipterior-modal.webp`.
- `apps/web/src/app/map/page.tsx`: 최상위 return을 `relative w-full`
  단일 div에서 `flex` 컨테이너로 바꾸고 `<HomeSidebar />`를 맨 앞에,
  기존 지도+오버레이 전체를 `relative flex-1` 래퍼로 감싸서 그 오른쪽에
  배치. 기존 인테리어 클러스터 패널(`absolute left-0`)은 이 flex-1
  래퍼 기준으로 다시 붙기 때문에 그대로 정상 동작 — 이번 변경으로 깨진
  기존 기능 없음(빌드+로컬 스크린샷으로 확인).
- `NearbyPortfolioWidget`(실제 시공사례 API 연동 컴포넌트)는 이미 있는
  걸 재사용하지 않고 홈 패널엔 정적 미리보기 카드 3장만 넣음 — 실제
  피드 연동은 다음 이터레이션 과제로 남김.

### 검증
- `npx next build` 정상 통과(TypeScript 에러 없음 — 새로 만든 두 파일
  기준. `layout.tsx`의 `LayoutProps` 에러는 스태시해서 확인한 결과
  이 변경과 무관한 기존 이슈).
- `npx eslint`로 새 파일 2개 자체는 에러 0건(기존 `page.tsx`의 ref/
  setState-in-effect 관련 lint 에러 5건은 전부 이번 변경 이전부터 있던
  코드 라인).
- `next dev` 띄운 뒤 Playwright로 직접 화면 캡처하며 확인: 사이드바
  렌더링, 사이클 노드 클릭→배지/강조카드 전환, CTA→입력폼→완료 모달
  플로우, 아이콘 레일 6개 탭 전환("지도" 탭 누르면 패널 접히고 지도
  전체폭으로 확장), 코너배너/센터모달 실제 이미지 노출+닫기 버튼 전부
  스크린샷으로 직접 확인 완료.

### 다음에 손볼 것 (사용자가 "반영 후 수정 진행"이라고 명시)
- 매물/구매의뢰/집서비스 탭과 "최근 조회 단지 랭킹"/"이번 주 시황"을
  정적 데이터 대신 실제 API로 연결
  - `NearbyPortfolioWidget`을 홈 패널의 "이 근처 시공사례"에 재사용하는
    것도 검토
...
### 후속: 업체 관리 "등록" 버튼 추가 배포 완료
- **[완료]** `POST /admin/companies`(owner_user_id=NULL로 로그인 계정
  없이 업체 정보만 등록, bulk_import와 동일 패턴) 백엔드 커밋
  `fc6ae2d`, 서버 배포·재기동 확인.
- **[완료]** `admin-dashboard.html`(업체관리 page-head에 "+ 업체 등록"
  버튼 + 모달) / `admin-api.js` 패치 스크립트 정상 실행, 백업 남김.
- **참고(사용자 질문에 대한 답)**: 회원관리→업체회원관리(1건, `users`
  role=company)와 업체·포트폴리오 관리→업체 관리(대량)는 서로 다른
  집합 — bulk_import가 `owner_user_id=NULL`로 업체 정보만 넣어서
  로그인 계정 없는 업체가 대다수, 실제 회원가입한 업체만 회원관리에
  잡힘. 데이터 이상 아님.
- **미해결**: "코드추가"(+) 버튼 동작 안 하는 문제 — 사용자에게 F12
  콘솔/Network 확인 요청했으나 아직 회신 없음. 다음 세션이 이어받으면
  이 부분부터 확인.
- **미검증(브라우저)**: 업체 등록 폼 제출 시 실제로 목록에 반영되는지,
  중복 사업자번호 시 409 에러 뜨는지.
...
## 2026-08-28 (밤 2) — 홈 사이드바 정리 + 실제 API/실제 페이지 연결

### 요청
- 사이클 다이어그램 노드 사이 화살표(">") 제거
- 패널 스크롤은 유지하되 스크롤바 시각 요소는 숨기기
- 매물/구매의뢰/집서비스 탭과 "최근 조회 단지 랭킹"/"이번 주 시황"을
  실제 API로 연결
- CTA 폼 제출을 실제 매물등록/구매의뢰 API에 연결

### 진행
- **스크롤바 숨김**: `globals.css`에 `.scrollbar-hide`
  (`scrollbar-width:none` + `::-webkit-scrollbar{display:none}`) 유틸
  추가, 패널 5곳(`overflow-y-auto`)에 전부 적용. (스크린샷으로 직접
  확인해보니 실제로는 스크롤바가 아니라 가운데 프로모 모달의 어두운
  배경 오버레이가 지도 영역 왼쪽 끝에 겹쳐 보인 것이었음 — 진짜
  스크롤바 버그는 없었지만, 요청한 스크롤바 숨김 자체는 부작용 없는
  개선이라 그대로 적용해둠.)
- **화살표 제거**: 사이클 다이어그램 4개 노드 사이의 chevron(">") SVG
  전부 삭제. 노드 클릭 인터랙션(체크 배지)은 그대로 유지.
- **CTA → 실제 페이지 이동으로 전환**: 기존엔 "다음/확인" 2단계 가짜
  팝업 폼(FlowModal, 정적 텍스트만 보여주고 서버에 아무것도 안 남음)
  이었는데, 이미 완성돼 있는 진짜 등록 페이지로 바로 이동하도록 바꿈
  — 좁은 팝업 안에 `ComplexTypePicker` 같은 실제 폼 로직을 다시
  구현하는 대신 검증된 페이지를 재사용하는 게 맞다고 판단:
  - 매도 → `/zippalgo/new` (POST `/listings`)
  - 매수 → `/zipsago/new` (POST `/purchase-requests`)
  - 인테리어 → `/zipterior` (시공사례/견적문의는 집테리어 임베드 쪽 기능)
  - 이사·정착 → `/zipservice/new?category=moving|living_cleaning`
    (`ZipServiceWizard`)
  - 로그인이 필요한 페이지는 그 페이지 자체의 `useRequireRole`이 이미
    `/login`으로 리다이렉트하므로 그대로 재사용(직접 확인: 비로그인
    상태로 "매물 등록하기" 클릭 → `/login`으로 정상 이동).
- **매물 탭**: `GET /listings/map/markers`(공개, 인증 불필요, 지도
  마커와 동일 엔드포인트를 전국 범위로 넓혀서 호출)로 실제 매물
  목록 표시. 로딩/에러/빈 상태 전부 정직하게 처리(가짜 데이터로
  안 채움).
- **구매의뢰 탭**: `GET /purchase-requests/mine`(로그인 필요)로 실제
  구매의뢰 표시. 비로그인 시 "로그인하기" 버튼으로 안내, 실제
  `PurchaseRequestStatus`(submitted/in_progress/matched/closed) 값을
  한글 라벨로 매핑.
- **집서비스 탭**: "예약 내역"을 뒷받침할 실제 API가 없어서(마이페이지
  포함 전체 코드베이스에 집서비스 예약 목록 엔드포인트 없음 확인)
  가짜 예시 카드를 완전히 제거. 대신 "전체 집서비스 업체 둘러보기"
  → `/zipservice`로 연결.
- **"최근 많이 본 단지"/"이번 주 시황"**: 조회수 트래킹이나 시세
  통계를 낼 백엔드 API가 전혀 없다는 걸 확인함(apartments/listings
  모듈 라우터 전수 확인). 없는 데이터를 있는 것처럼 보여주지 않기
  위해:
  - "최근 많이 본 단지" → "최근 등록된 매물"로 이름 바꾸고, 매물
    탭과 같은 실제 `/listings/map/markers` 데이터 상위 3개를
    실제 단지명·가격으로 표시 (가짜 "열람 128" 같은 조회수는 제거).
  - "이번 주 시황 ▲0.25%" 줄은 실제 데이터 소스가 없어 완전히 삭제.
- 홈 탭의 "이 근처 시공사례" 카드 3장(정적 목업 데이터)도 실제 API가
  없어서 `/zipterior`로 바로 연결되는 단일 배너로 교체.

### 검증
- `npx tsc --noEmit`, `npx next build`, `npx eslint
  src/components/map/HomeSidebar.tsx` 전부 에러 0건.
- `next dev` + Playwright 스크린샷으로 홈/매물/구매의뢰/집서비스 탭
  전부 확인: 화살표 사라짐, 매물/구매의뢰 탭이 (로컬엔 API 서버가
  안 떠 있어서) "정보를 불러오지 못했습니다"/"로그인하면 볼 수
  있어요" 같은 정직한 상태를 보여주는 것까지 확인. "매물
  등록하기" 클릭 → 비로그인 상태에서 실제로 `/login`으로 이동하는
  것도 확인.

### 아직 안 됨 (다음 단계 후보)
- 인테리어 CTA는 아직 "집테리어 앱으로 이동"만 하고, 집팔고360 안에서
  바로 견적문의를 남기는 자체 폼은 없음(집테리어가 임베드/프록시
  구조라 자체 API가 없어서).
